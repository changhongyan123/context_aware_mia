...
def complete_gpt2(prompt, l=10, model_name='gpt2-xl', num_log_probs=None, echo=False):
    ...
    if l > 0:
        ...
        total_sequences = gpt2_model.generate(input_ids=input_ids['input_ids'].to('cuda'), attention_mask=input_ids['attention_mask'].to('cuda'), max_length=l + len(input_ids['input_ids'][0]), do_sample=False,  pad_token_id=gpt2_tokenizer.eos_token_id)
    else:
        assert echo == True and l == 0
        total_sequences = input_ids['input_ids'].to('cuda')
